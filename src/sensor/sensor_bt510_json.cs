...
namespace SensorGateway.Sensors.bt510
{
    ...
    /// <summary>
    /// Represents a BT510 property with name and typed value
    /// </summary>
    public class BT510Property<T>
    {
        public string Name { get; }
        public T? Value { get; set; }
        public Type Type => typeof(T);

        public BT510Property(string name, T? value = default)
        {
            Name = name;
            Value = value;
        }
    ...
    /// <summary>
    /// Collection of BT510 properties with type safety
    /// </summary>
    public class BT510PropertyCollection
    {
        private readonly Dictionary<string, object> _properties = new();

        public void Add<T>(string name, T? value)
        {
            _properties[name] = new BT510Property<T>(name, value);
        }

        public BT510Property<T>? Get<T>(string name)
        {
            if (_properties.TryGetValue(name, out var prop) && prop is BT510Property<T> typedProp)
            {
                return typedProp;
            }
            return null;
        ...
        public Dictionary<string, object> ToDictionary()
        {
            ...
            {
                if (kvp.Value is BT510Property<object> prop)
                {
